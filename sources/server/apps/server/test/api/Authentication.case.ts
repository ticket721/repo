...
export async function register(context: any): Promise<void> {
    ...
    expect(auth_res.data.user.address).toEqual(toAcceptedAddressFormat(wallet.address));
    ...
    await expect(sdk.localRegister('test2@test.com', password, 'mortimr2', wallet, () => {})).rejects.toMatchObject({
        response: {
            data: {
                statusCode: 409,
                name: 'Conflict',
...
export async function web3register(context: any): Promise<void> {
    ...
    await expect(sdk.web3Register(email, username, registerPayload[0], wallet.address, signedPayload.hex)).rejects.toMatchObject({
        response: {
            data: {
                message: 'email_already_in_use',
                statusCode: 409,
    ...
    await expect(sdk.web3Register('test2@test.com', username, registerPayload[0], wallet.address, signedPayload.hex)).rejects.toMatchObject({
        response: {
            data: {
                message: 'username_already_in_use',
                statusCode: 409,
    ...
    await expect(sdk.web3Register('test2@test.com', 'mortimr2', registerPayload[0], (await createWallet()).address, '0x')).rejects.toMatchObject({
        response: {
            data: {
    ...
    await expect(sdk.web3Register('test2@test.com', 'mortimr2', registerPayload[0], (await createWallet()).address, signedPayload.hex)).rejects.toMatchObject({
        response: {
            data: {
    ...
    await expect(sdk.web3Register('test2@test.com', 'mortimr2', conflictRegisterPayload[0], wallet.address, conflictSignedPayload.hex)).rejects.toMatchObject({
        response: {
            data: {
                message: 'address_already_in_use',
                statusCode: 409,
    ...
    await expect(sdk.web3Register(future_email, future_username, future, third_wallet.address, future_signature.hex)).rejects.toMatchObject({
        response: {
            data: {
    ...
    await expect(sdk.web3Register(past_email, past_username, past, third_wallet.address, past_signature.hex)).rejects.toMatchObject({
        response: {
            data: {
