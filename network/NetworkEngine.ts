...
export class NetworkEngine extends Engine<NetworkConfig> {
    ...
    async run(): Promise<void> {
        ...
        for (let idx = 0; idx < 20; ++idx) {
            try {
                await eth_node_liveness_check(this.config.connector, this.config.host, this.config.port, this.config.protocol, this.config.headers || [], this.config.path);
            } catch (e) {
                if (idx < 19) {
                    network_log.warn();
        ...
        try {
            await eth_node_check_net_id(this.config.connector, this.config.host, this.config.port, this.config.protocol, this.config.network_id, this.config.headers || [], this.config.path);
        } catch (e) {
