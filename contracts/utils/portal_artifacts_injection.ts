...
/**
 * Utility that takes an artifacts directory and creates an artifact containing all directory informations
 * inside the portal
 *
 * @param path_to_artifacts
 * @param module_name
 */
export function portal_artifacts_injection(path_to_artifacts: string, module_name: string): void {
    const files = fse.readdirSync(path_to_artifacts);

    const artifact = {};

    for (const file of files) {
        const content = JSON.parse(fse.readFileSync(path.join(path_to_artifacts, file)));
