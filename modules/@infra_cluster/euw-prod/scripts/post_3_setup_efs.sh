...
EXISTING_FS_CHECK=$(aws efs describe-file-systems --output json | jq '.FileSystems[] | select(.Tags[].Key=="Cluster" and .Tags[].Value=="euw-prod")' | jq '.FileSystemId')
CLUSTER_NAME=euw-prod
VPC_ID=$(aws eks describe-cluster --name $CLUSTER_NAME --query "cluster.resourcesVpcConfig.vpcId" --output text)
CIDR_BLOCK=$(aws ec2 describe-vpcs --vpc-ids $VPC_ID --query "Vpcs[].CidrBlock" --output text)

MOUNT_TARGET_GROUP_NAME="eks-euw-prod-efs"
...
then
  FILE_SYSTEM_ID=$(aws efs create-file-system --tags "Key=Cluster,Value=euw-prod" | jq --raw-output '.FileSystemId')
else
  FILE_SYSTEM_ID="${EXISTING_FS_CHECK}"
fi
