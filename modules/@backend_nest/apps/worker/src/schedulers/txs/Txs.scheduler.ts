                ...
                if (pendingTransactions.response.hits.total > 0) {
                    for (const hit of pendingTransactions.response.hits.hits) {
                        ...
                        if (isTrackingId(parsed.transaction_hash)) {
                            ...
                            parsed.from_ = toAcceptedAddressFormat(txInfos.response.from);
                            parsed.to_ = toAcceptedAddressFormat(txInfos.response.to);
            ...
            if (pendingTransactions.response.hits.total > 0) {
                ...
                for (const hit of pendingTransactions.response.hits.hits) {
                    ...
                    if (isTrackingId(parsed.transaction_hash)) {
                        ...
                        parsed.real_transaction_hash = parsed.transaction_hash;
