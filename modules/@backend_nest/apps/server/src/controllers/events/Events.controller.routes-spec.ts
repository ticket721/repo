...
import { createEvent, createPaymentIntent, failWithCode, getSDKAndUser, getUser, waitForActionSet, waitForTickets } from '../../../test/utils';
        ...
        describe('withdraw (POST /events/:eventId/withdraw)', function() {
            ...
            test('should properly recover tokens from user purchases', async function() {
                ...
                await waitForTickets(sdk, token, user.address, (tickets: TicketEntity[]): boolean => {
                    return tickets.length === 3 && tickets.filter((ticket: TicketEntity): boolean => ticket.status === 'ready').length === 3
                });
