...
export default function(getCtx: () => { ready: Promise<void> }) {
    return function() {
        ...
        describe('resolveCartWithPaymentIntent (POST /checkout/cart/resolve/paymentintent)', function() {
            ...
            test('should create, fill and commit cart, then fail resolving card paymetn and revert reserved seat count', async function() {
                const {
                    sdk,
                    token,
                    user,
                ...
                await invalidateCardPayment(await getPIFromCart(sdk, token, actionSetId));
                ...
                await waitForActionSet(sdk, token, checkoutActionSetId, (as: ActionSetEntity): boolean => {
                    return as.current_status === 'event:error';
                });

                await waitForCategory(sdk, token, event.categories[0], (cat: CategoryEntity): boolean => {
                    return cat.reserved === reservedCount;
                });
            });
        });
    };
