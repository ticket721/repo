...
export class T721TokenService extends ContractsControllerBase {
    ...
    async getMinter(): Promise<[string, string]> {
        if (this.minter === null) {
            ...
            try {
                const mintersList = JSON.parse(this.fsService.readFile(path.join(this.contractsOptions.artifact_path, 'minter_management', 'artifacts', 'minters.json')));
                const identitiesList = JSON.parse(this.fsService.readFile(path.join(this.contractsOptions.artifact_path, 'minter_management', 'artifacts', 'identities.json')));
                this.minter = mintersList[minterAddressIndex];
                this.identity = identitiesList[minterAddressIndex];
            } catch (e) {
                this.shutdownService.shutdownWithError(new Error(`Unable to resolve minter address required to create tokens: ${e.message}`));
            }
    ...
    async generateAuthorization(grantee: string, amount: string): Promise<ServiceResponse<[AuthorizationEntity, string, string, string]>> {
        ...
        const signer = await new MintTokensAuthorization(
            t721AdminAddress,
            net
        );
        ...
        const hash = MintTokensAuthorization.encodeAndHash(
            grantee,
            amount,
            minter,
            code
        ...
        const signedPayload = await signer.sign(
            this.rocksideService.getSigner(minter),
            payload
        );
        ...
        return {
            error: null,
            response: [
                authorizationCreationRes.response,
                code,
                sender,
                minter
            ]
