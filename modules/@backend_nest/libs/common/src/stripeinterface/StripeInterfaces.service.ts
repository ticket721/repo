...
export class StripeInterfacesService extends CRUDExtension<StripeInterfacesRepository, StripeInterfaceEntity> {
        ...
        if (stripeInterface.connect_account) {
            ...
            if (force || StripeInterfacesService.shouldUpdateAccountInfos(stripeInterface)) {
                ...
                const newValues: Partial<StripeInterfaceEntity> = {
                    ...
                    connect_account_external_accounts: account.external_accounts.data.map((value): ConnectAccountExternalAccount => ({
                        id: value.id,
    ...
    async bindAccountToUserInterface(user: UserDto, account: Stripe.Account): Promise<ServiceResponse<StripeInterfaceEntity>> {
    ...
    static containsExternalAccountFingerprint(stripeInterface: StripeInterfaceEntity, fingerprint: string): boolean {
        if (stripeInterface.connect_account_external_accounts && stripeInterface.connect_account_external_accounts.length) {
    ...
    async addExternalAccountToUserInterface(user: UserDto, bankAccountToken: string): Promise<ServiceResponse<StripeInterfaceEntity>> {
        ...
        if (StripeInterfacesService.containsExternalAccountFingerprint(stripeInterfaceRes.response, bankAccount.bank_account.fingerprint)) {
            return {
