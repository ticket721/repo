  ...
  return (
    <Container>
      { dates.map((date: { start: Date, end: Date }, i) => (
        <React.Fragment key={`resales-by-date-${i}`}>
          <SelectInput
            className={'select'}
            options={options}
            multiple
            ...
            onChange={(d: any, e: any) => {
              ...
              if (e.action === 'remove-value') {
                const newValues = formik.values.dates[index].dates.map(
                  ...
                  d => {
                    if (e.removedValue.value === d.eventDate) {
                      return ({...d, resalesStart: undefined, resalesEnd: undefined});
                    } else {
                      return d;
                    }
                  }
                ...
                const newValues = formik.values.dates[index].dates.map(
                  // @ts-ignore
                  d => ({...d, resalesStart: undefined, resalesEnd: undefined})
                );
                newDates[i] = [];
                setValues(newDates);
                formik.setFieldValue(`dates[${index}].dates`, newValues);
                ...
                const newValues = formik.values.dates[index].dates.map(
                  ...
                  d => {
                    if (newDates[i].find(v => v.value === d.eventDate)) {
                      return ({...d, resalesStart: date.start, resalesEnd: date.end });
                    } else {
                      return d;
                    }
                  }
          ...
          <Line>
            <CustomDatePicker
              ...
              onChange={(e) => {
                const newDates = formik.values.dates[index].dates.map(
                  // @ts-ignore
                  d => values[i].find(v => v.value === d.eventDate) ? ({...d, resalesStart: e}) : (d)
                );
            ...
            <CustomDatePicker
              ...
              onChange={(e) => {
                const newDates = formik.values.dates[index].dates.map(
                  // @ts-ignore
                  d => values[i].find(v => v.value === d.eventDate) ? ({...d, resalesEnd: e}) : (d)
                );
                formik.setFieldValue(`dates[${index}].dates`, newDates);
                const newValues = [...dates];
                newValues[i].end = e;
