...
export const CategoriesSubMenu: React.FC = () => {
    ...
    }, [params?.dateId, history.location]);
    ...
    return <CategoriesMenuContainer>
        ...
        onClick={(e) => {
            if ((e.target as any).className.includes('chevron')) {
                if (!collapsed) {
                    history.push(`/event/${params.eventId}/date/${params.dateId}/dates-typology`);
                } else {
                    history.push(`/event/${params.eventId}/date/${params.dateId}/categories`);
                }
        ...
        <CategoriesContainer collapsed={collapsed} categoriesCount={categoriesResp.data.categories.length}>
            {
                categoriesResp.data.categories.map(category => (
                    <Link
                    key={category.id}
                    selected={params?.categoryId === category.id}
                    onClick={() => history.push(`/event/${params.eventId}${params.dateId ? `/date/${params.dateId}` : ''}/category/${category.id}`)}>
        ...
        onClick={() => {
            setCollapsed(false);
            history.push(`/event/${params.eventId}${params.dateId ? `/date/${params.dateId}` : ''}/category`, { dates: [params.dateId] });
        }}>
            <Plus>+</Plus>
