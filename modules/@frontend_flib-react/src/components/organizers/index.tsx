...
const Organizer = (props: OrganizerProp) => {
    return (
        <ListItem>
            <ImgContainer>
                <img
                    src={props.organizer.image ? props.organizer.image : 'assets/images/t721--logo.png'}
            ...
            <div>
                ...
                {props.organizer.status && (
                    <span>
                        {props.organizer.status} {props.organizer.acceptedOn && `on ${props.organizer.acceptedOn}`}
                    </span>
                )}
            </div>
            {props.removable && (
...
const InviteOrganizers: React.FunctionComponent<InviteOrganizersProps> = (
    props: InviteOrganizersProps,
): JSX.Element => {
    return (
            ...
            <StyledContainer>
                ...
                <OrganizersList>
                    <Organizer
                        ...
                        removeUser={props.handleRemove}
                    />
                    {props.organizers.map((organizer) => {
                        return (
                            <Organizer
                                key={organizer.id ? organizer.id : organizer.email}
                                organizer={organizer}
                                removable
                                removeUser={props.handleRemove}
                            />
