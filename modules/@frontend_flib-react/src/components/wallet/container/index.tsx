...
export const WalletContainer: React.FunctionComponent<WalletContainerProps & { className?: string }> = (
    props: WalletContainerProps,
): JSX.Element => {
    return (
        <Container className={props.className} toggle={props.showWallet}>
            <Nav>
                {props.showPrev && <ArrowIcon icon={'arrow'} size={'16px'} color={'rgba(255, 255, 255, 0.9)'} />}
                <h2>{props.title}</h2>
