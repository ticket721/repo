    ...
    return (
        <Container>
            <div className={'row aic jcsb'}>
                <h3>{props.title}</h3>
                {props.terms.length > 0 && <ClearButton onClick={props.clearAll}>{props.clearLabel}</ClearButton>}
            </div>
            ...
            <TermList>
                {props.terms.length ? (
                    props.terms.map((term: string) => {
                        return (
                            <Term key={slugify(term)}>
                                <Icon icon={'search'} size={'24px'} color={'rgba(255, 255, 255, 0.38)'} />
                                {term}
                            </Term>
                        );
                    })
