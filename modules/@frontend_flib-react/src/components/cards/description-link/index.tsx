...
export const DescriptonLink: React.FunctionComponent<DescriptionLinkProps> = (
    props: DescriptionLinkProps,
): JSX.Element => {
    return (
        <CardContainer removeBg={props.removeBg}>
            <div className={'row aic jcsb'}>
                <Content>
                    {props.subtitle && <Subtitle className={'uppercase'}>{props.subtitle}</Subtitle>}
                    <H3>{props.title}</H3>
                ...
                {props.link && (
                    <a href={props.link}>
                        <Icon icon={'rightArrow'} size={'16px'} color={'rgba(255, 255, 255, 0.9)'} />
                    </a>
                )}
            </div>
        </CardContainer>
