...
export const useRequest = <ReturnType>(call: RequestParams, initialUuid: string): RequestBag<ReturnType> => {
    ...
    const data = useSelector(
        (state: AppState) => state.cache.items[key]?.data,
        shallowEqual,
    );

    const error = useSelector(
        (state: AppState) => state.cache.items[key]?.error,
        shallowEqual,
    );

    const loading = useSelector(
        (state: AppState) => !state.cache.items[key],
    );
    ...
    const unregisterEntity = useCallback(
        (uuid: string): void =>
            void dispatch(UnregisterEntity(key, uuid)),
