...
const SetTokenReducer: Reducer<AuthState, ISetToken> =
    (state: AuthState, action: ISetToken): AuthState => ({
        ...state,
        token: action.token,
    });

const SetUserReducer: Reducer<AuthState, ISetUser> =
    (state: AuthState, action: ISetUser): AuthState => ({
        ...state,
        user: {
...
const LogoutReducer: Reducer<AuthState, ILogout> =
    (state: AuthState, action: ILogout): AuthState => ({
        token: null,
        user: null,
        errors: null,
        loading: false,
...
const SetErrorsReducer: Reducer<AuthState, ISetErrors> =
    (state: AuthState, action: ISetErrors): AuthState => ({
        ...state,
        errors: action.errors,
    });

const ResetErrorsReducer: Reducer<AuthState, IResetErrors> =
    (state: AuthState, action: IResetErrors): AuthState => ({
        ...state,
        errors: null,
    });

const SetLoadingReducer: Reducer<AuthState, ISetLoading> =
    (state: AuthState, action: ISetLoading): AuthState => ({
        ...state,
        loading: action.value,
});

const SubmissionReducer: Reducer<AuthState, ILocalRegister | ILocalLogin> =
    (state: AuthState, action: ILocalRegister | ILocalLogin): AuthState => ({
        ...state,
        errors: null,
        submit: true,
    });
...
export const AuthReducer: Reducer<AuthState, AuthAction> =
    (state: AuthState = authInitialState, action: AuthAction): AuthState => {
        switch (action.type) {
