...
const GetDeviceReducer: Reducer<UserPropertiesState, IGetDevice> =
    (state: UserPropertiesState, action: IGetDevice): UserPropertiesState => ({
        ...state,
        loading: true
    });

const SetDeviceInfosReducer: Reducer<UserPropertiesState, ISetDeviceInfos> =
    (state: UserPropertiesState, action: ISetDeviceInfos): UserPropertiesState => ({
...
const GetCityReducer: Reducer<UserPropertiesState, IGetCity> =
    (state: UserPropertiesState, action: IGetCity): UserPropertiesState => ({
        ...state,
        loading: true
    });

const SetCityReducer: Reducer<UserPropertiesState, ISetCity> =
    (state: UserPropertiesState, action: ISetCity): UserPropertiesState => ({
        ...userPropertiesInitialState,
        user: {
            ...state.user,
            city: action.city
...
export const UserPropertiesReducer: Reducer<UserPropertiesState, UserPropertiesAction> =
    (state: UserPropertiesState = userPropertiesInitialState, action: UserPropertiesAction): UserPropertiesState => {
        switch (action.type) {
